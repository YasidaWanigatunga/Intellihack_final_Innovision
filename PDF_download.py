...
def download_pdf(paper):
    ...
    try:
        ...
        if response.status_code == 200:
            pdf_path = f"{paper['title']}.pdf".replace("/", "_").replace("\\", "_")
            with open(pdf_path, 'wb') as f:
                f.write(response.content)
            logger.info(f"PDF downloaded and saved as {pdf_path}")
        else:
...
def main():
    ...
    while True:
        download_choice = input("Do you want to download the PDF for any paper? (yes/no): ")
        if download_choice.lower() == 'no':
            break
        paper_title = input("Enter the title of the paper you want to download: ")
        selected_paper = next((paper for paper in all_papers if paper['title'] == paper_title), None)
        if selected_paper:
            download_pdf(selected_paper)
        else:
            print("Paper not found. Please enter a valid title.")
